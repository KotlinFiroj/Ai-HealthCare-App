package com.mediai.enterprise.core.navigation

/**
 * [MediAINavDestinations]
 * Centralized navigation destinations for the entire application.
 */
object MediAINavDestinations {
    const val AUTH_ROUTE = "auth_route"
    const val HOME_ROUTE = "home_route"
    const val CHAT_ROUTE = "chat_route"
    const val REPORTS_ROUTE = "reports_route"
    const val REPORT_DETAIL_ROUTE = "report_detail_route"
    const val APPOINTMENTS_ROUTE = "appointments_route"
    const val QR_CHECKIN_ROUTE = "qr_checkin_route"
    const val HOSPITAL_MAP_ROUTE = "hospital_map_route"
    const val BIOMETRIC_ENROLLMENT_ROUTE = "biometric_enrollment_route"
    const val PAYMENT_ROUTE = "payment_route"
    const val CONSULTATION_ROUTE = "consultation_route"
}
